...
KillConjugates{Nr}(parent::P) where {Nr,P} = KillConjugates{Nr,P}(parent)
...
Base.size(k::KillConjugates) = size(k.parent)
...
function Base.iterate(k::KillConjugates, (take, state))
    ...
    isnothing(result) && return nothing
    if iszero(take)
        return zero(result[1]), (one(take), result[2])
    else
        return result[1], (take - one(take), result[2])
    end
end