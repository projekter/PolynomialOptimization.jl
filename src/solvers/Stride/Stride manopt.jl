...
# This is the working implementation with BFGS coming from Optim and an automatic local optimization constructed using ManOpt
# (though the semidefinite part is broken)
using Printf
...
function get_varrepr!(sst::StrideState{M}, gröbner_basis, grouping::AbstractVector{M}, row, col, constraint::P) where {P,M}
    @inbounds rowcol = grouping[row] * grouping[col]
    ...
    @inbounds if length(constr_terms) == 1
...
function prepare_relaxation(problem::PolyOptProblem{P,M,V}, groupings::Vector{<:Vector{<:AbstractVector{M}}},
    ...
    @inbounds let tmp = Vector{Int}(undef, maxdim * (maxdim +1) ÷ 2), m = sst.constraints
    ...
    local AAstarinv
    @inbounds let m = sst.constraints
        AAstarcolsize = 0
    ...
    let
        ...
        lastidx = 0
        @inbounds for term in problem.objective
        ...
        for (i, dim, Iᵢ, Vᵢ) in zip(1:lastidx, sst.psds, I, Vs)
            @inbounds C[i] = PackedMatrix(dim, sparsevec(Iᵢ, Vᵢ, dim * (dim +1) ÷ 2))
        end
    end
...
"""
    stride_default_local(problem::PolyOptProblem, tol, precision::Integer=0; verbose::Bool=false)

Generates a default local optimization routine that relies on Manopt's local AugmentLagrangianMethod. Sometimes (see
[#232](https://github.com/JuliaManifolds/Manopt.jl/issues/232)), Manopt fails due to internal loss of precision. By specifying
a positive integer as precision, all machine precision values will be converted to BigFloats of the desired precision and the
local optimization will use this internal precision.
"""
function stride_default_local(problem::PolyOptProblem, tolerance, precision::Integer=0; verbose::Bool=false, kwargs...)
    @assert(precision ≥ 0)
    ...
    if iszero(precision)
        ...
        R = BigFloat
        lastprecision = Base.MPFR.precision(BigFloat)
        setprecision(precision)
    end
    tol::R = R(tolerance)
    # First, we need to construct the manifold. Usually, this is just ℝ^n, but in the case of PSD constraints, we additionally
    # create the necessary variables for the matrices which are then in 𝕊^d and use equality constraints to force them to their
    # values.
    ...
    constrs_eq = Vector{polynomial_type(problem,R)}(undef, equality_constraints + psd_constraints)
    constrs_ineq = Vector{polynomial_type(problem,R)}(undef, inequality_constraints)
    manis = Manifolds.AbstractManifold{Manifolds.ℝ}[Manifolds.Euclidean(nvars)]
    sizehint!(manis, 1 + npsd)
    i_eq, i_ineq, i_psd = 1, 1, 1
    @inbounds for c in problem.constraints
        if c.constraint isa AbstractMatrix
            for mj in 1:size(c.constraint, 1), mi in 1:mj
                constrs_eq[i_eq] = c.constraint[mi, mj] - psd_variables[i_psd]
                i_eq += 1
                i_psd += 1
            end
            # We don't know about the rank, so we construct the full-rank one. This can still lead to lower-rank solutions,
            # although the geometry breaks down.
            push!(manis, Manifolds.SymmetricPositiveSemidefiniteFixedRank(size(c.constraint, 1), size(c.constraint, 1)))
        elseif c.type == pctNonneg
    ...
    point = Manifolds.ProductRepr(map((N) -> R.(rand(N)), mani.manifolds)...) # instead of rand(mani)
    point_assembled = Vector{R}(undef, length(all_variables))
    point_dissembled = similar(point_assembled)
    assemble!::Function = p::Manifolds.ProductRepr -> let i = 1, point_assembled = point_assembled
    ...
    dissemble!::Function = to::Manifolds.ProductRepr -> let i = 1, point_dissembled = point_dissembled
...
function mlbfgs!(ssd::StrideData{R}, α::Vector{R}, ρs::Vector{R}, S::Matrix{R}, Y::Matrix{R}, K::R, tol::R, μ::R, ρ::R, τ₁::R,
    ...
    @inbounds tmpm, ∇ϕ, tmpv = ssd.tmpm1, ssd.tmpv, view(ssd.tmpvlong, 1:ssd.d)
    # Iterate the following steps for k = 1, ...
    k = 1
...
function η(ssd::StrideData{R}, details::Union{Type{Val{false}},Type{Val{true}}}=Val{false}) where {R}
    ...
    @inbounds for (i, (Sᵢ, Xᵢ, tmpmᵢ, Astarᵢ)) in enumerate(zip(S, X, tmpm, Astar))
